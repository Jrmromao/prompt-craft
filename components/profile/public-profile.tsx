...
export function PublicProfile({ user, followerCount, followingCount, recentActivity }: PublicProfileProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background to-background/95">
      {/* Hero Section */}
      <div className="relative border-b border-border/40 bg-gradient-to-b from-blue-500/5 to-blue-500/5">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,_var(--tw-gradient-stops))] from-blue-500/20 via-transparent to-transparent animate-gradient-x" />
        <div className="container relative mx-auto px-4 py-16 flex flex-col items-center text-center">
          <div className="relative mb-4">
            <div className="absolute -inset-2 rounded-full bg-gradient-to-r from-blue-500 to-blue-500 opacity-30 blur-lg animate-pulse" />
            <Avatar className="relative h-32 w-32 ring-4 ring-background shadow-xl">
              <AvatarImage src={user.imageUrl || ''} alt={user.username || ''} />
              <AvatarFallback className="bg-gradient-to-br from-blue-500 to-blue-500 text-2xl">
          ...
          <h1 className="text-4xl font-extrabold tracking-tight bg-gradient-to-r from-blue-500 to-blue-500 bg-clip-text text-transparent flex items-center justify-center gap-2">
          ...
          <div className="flex items-center justify-center gap-3 mb-4">
            {website && (
              <a href={website} target="_blank" rel="noopener noreferrer" className="hover:underline text-blue-600 flex items-center gap-1"><Globe className="h-4 w-4" />Website</a>
            )}
            {twitter && (
          ...
          <div className="flex flex-wrap items-center justify-center gap-6 bg-white/60 dark:bg-black/30 rounded-xl shadow-lg px-8 py-4 mb-6 mt-2 backdrop-blur-md">
            <div className="flex flex-col items-center">
              <span className="text-xl font-bold text-blue-600">{followerCount}</span>
              <span className="text-xs text-muted-foreground">Followers</span>
            </div>
            <div className="flex flex-col items-center">
              <span className="text-xl font-bold text-blue-500">{followingCount}</span>
              <span className="text-xs text-muted-foreground">Following</span>
            </div>
            <div className="flex flex-col items-center">
              <span className="text-xl font-bold text-blue-600">{publicTemplates.length}</span>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 mb-2">
            <button className="inline-flex items-center justify-center rounded-full bg-gradient-to-r from-blue-500 to-blue-500 px-8 py-3 text-base font-semibold text-white shadow-lg transition-all hover:shadow-blue-500/25 hover:scale-105">
              <Users className="mr-2 h-5 w-5" />Follow
            </button>
            <button className="inline-flex items-center justify-center rounded-full border border-blue-500/20 bg-background/50 px-8 py-3 text-base font-semibold text-blue-500 backdrop-blur-sm transition-all hover:bg-blue-500/10">
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="grid gap-8 md:grid-cols-2">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-2xl font-semibold tracking-tight bg-gradient-to-r from-blue-500 to-blue-500 bg-clip-text text-transparent">
                Public Templates
              </h2>
            ...
            <div className="grid gap-4">
              {publicTemplates.map((template) => (
                <Link
                  ...
                  className="group relative overflow-hidden rounded-2xl border border-border/40 bg-white/80 dark:bg-card/50 p-6 backdrop-blur-md transition-all hover:border-blue-500/40 hover:shadow-xl hover:shadow-blue-500/10"
                >
                  <div className="flex items-start justify-between">
                    <div className="space-y-2">
                      <h3 className="font-semibold text-lg text-foreground group-hover:text-blue-500">
                        {template.name}
                        {template.type && (
                          <span className={cn("ml-2 px-2 py-0.5 rounded-full text-xs font-medium", template.type === 'few-shot' ? 'bg-blue-50 text-blue-500' : template.type === 'chain-of-thought' ? 'bg-yellow-100 text-yellow-700' : 'bg-blue-100 text-blue-600')}>{template.type}</span>
                        )}
                      </h3>
                    ...
                    <div className="flex items-center space-x-3">
                      <span className="flex items-center text-sm text-muted-foreground">
                        <Heart className="mr-1.5 h-4 w-4 text-blue-500" />
                  ...
                  <button className="absolute bottom-4 right-4 rounded-full bg-gradient-to-r from-blue-500 to-blue-500 px-4 py-2 text-xs font-semibold text-white shadow-lg opacity-0 group-hover:opacity-100 transition-all">View</button>
                </Link>
              ))}
            </div>
          </div>
        ...
        <div className="mt-12 max-w-2xl mx-auto">
          <h3 className="text-xl font-semibold mb-4 bg-gradient-to-r from-blue-500 to-blue-500 bg-clip-text text-transparent">Recent Activity</h3>
          <ul className="relative border-l-2 border-blue-200 dark:border-blue-800 pl-6 space-y-6">
            {recentActivity.map((activity, idx) => (
              <li key={idx} className="relative">
                <span className="absolute -left-3 top-1.5 flex h-3 w-3 items-center justify-center">
                  <span className="h-3 w-3 rounded-full bg-gradient-to-r from-blue-500 to-blue-500 shadow-lg"></span>
                </span>
