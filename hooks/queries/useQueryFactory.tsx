...
export function createGenericQuery<T extends { id?: string }, TCreateInput>(
  ...
  return function useGenericQuery(): UseGenericQueryResult<T, TCreateInput> {
    ...
    >({
      ...
      onSuccess: data => {
        toast.success(options?.successMessage || `${queryKey[0]} created successfully`);
        onClose?.();
        return data;
      },
      onSettled: () => {
    ...
    >({
      ...
      onError: (err, _, context) => {
        ...
        toast.error(options?.deleteErrorMessage || `Failed to delete ${queryKey[0]}`);
      },
      onSuccess: () => {
        toast.success(options?.deleteSuccessMessage || `${queryKey[0]} deleted successfully`);
      },
      onSettled: () => {
        queryClient.invalidateQueries({ queryKey });
      },
    ...
    >({
      ...
      onSuccess: () => {
        toast.success(options?.updateSuccessMessage || `${queryKey[0]} updated successfully`);
        onClose?.();
      },
      onSettled: () => {
        queryClient.invalidateQueries({ queryKey });
            ...
            onSuccess: (response: ActionResponse<T>) => {
              if (callbacks?.onSuccess) {
                if (callbacks.onSuccess.length > 0) {
                  (callbacks.onSuccess as (result: ActionResponse<T>) => void)(response);
                } else {
